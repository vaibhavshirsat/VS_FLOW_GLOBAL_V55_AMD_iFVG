...
# =========================
    ...
    if st.button("RUN 4H → 1H → 15M → 5M",use_container_width=True,key="asset_mtf_run"):
        st.session_state.asset_mtf_result=mtf_setup(sym)
    if "asset_mtf_result" in st.session_state:
        frames,overall=st.session_state.asset_mtf_result;cls="green" if overall=="BUY" else "red" if overall=="SELL" else "gold";st.markdown(f'<div class="panel"><span class="pill {cls}">OVERALL: {overall}</span></div>',unsafe_allow_html=True);st.dataframe(pd.DataFrame(frames),use_container_width=True,hide_index=True)
